...
const ThingAddForm = ({ onSubmit, goBack }) => (
  <Formik
    initialValues={{
      name: "Conveyor",
    ...
    validationSchema={Yup.object().shape({
      name: Yup.string().min(0).required().label("Thing name"),
      description: Yup.string().min(0).required().label("Thing description"),
      production_location: Yup.number()
        .min(0)
        .required()
        .label("Location on a production line"),
      device_group: Yup.string().min(0).required().label("Devices group"),
      device: Yup.string().min(0).required().label("Devices"),
    })}
    onSubmit={onSubmit}
  >
